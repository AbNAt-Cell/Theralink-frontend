...
export default function ChatDashboard() {
  ...
  const handleAudioCall = async () => {
    ...
    if (!peerId) {
      toast({ variant: 'destructive', title: 'Cannot start call', description: 'Unable to reach this user. They may not have used the messaging feature yet.' });
      return;
    }
  ...
  const handleSelectContact = async (contact: Contact) => {
    ...
    try {
      ...
      setRecipientPeerId(getPeerIdResponse?.peerId || null);
      if (response?._id) await markConversationAsRead(response._id);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredContacts?.map((contact: TransformedContact) => (
            <div
              key={contact?._id}
              onClick={() => {
                const contactData: Contact = { firstname: contact.firstname || "", lastname: contact.lastname || "", email: "", status: "", _id: contact._id, avatar: contact.avatar, isOnline: contact.isOnline };
      ...
      {selectedContact ? (
        <div className={`${isMobile ? (showSidebar ? "w-0 overflow-hidden" : "w-full") : "flex-1"} flex flex-col bg-gradient-to-b from-gray-50 to-gray-100 transition-all duration-300`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            ...
            {isTyping && <TypingIndicator userName={`${selectedContact?.firstname}`} />}
          ...
          <div className="bg-white border-t border-gray-200 p-4">
            <div className="flex items-center space-x-3">
              ...
              />
              <button onClick={handleSend} disabled={sending || !messageInput.trim()} className="p-2.5 bg-blue-600 text-white rounded-full hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-sm">
